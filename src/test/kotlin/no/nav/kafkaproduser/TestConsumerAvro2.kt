...
class TestConsumerAvro2(brokers: KafkaConfig) {
    ...
    private fun  createConsumer(): Consumer<String, MedlemKafkaDto> {

        return KafkaConsumer<String, MedlemKafkaDto>(inst2Config())
    }
    ...
    fun consume(ratePerSecond: Int) {
        ...
        while(true) {
            ...
            var records: ConsumerRecords<String, MedlemKafkaDto> = consumer.poll(Duration.ofSeconds(5))
            logger.info("Received ${records.count()} records")
            records.iterator().forEach {
                val medlemJson:MedlemKafkaDto = it.value()
                println(medlemJson)
            }
        }
